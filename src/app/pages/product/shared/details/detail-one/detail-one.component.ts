...
declare function sendCartConversion(): void;
declare var $: any;
...
export class DetailOneComponent implements OnInit {
	...
	addCart(event: Event, index = 0) {
		...
		if(this.token){
			...
			let variantProduct = this.product.variants[0];
			this.variant = variantProduct.id;
	...
	buyNowLink(event: Event)
	{
		...
		// else{
			// alert("buy now ");
			this.router.navigate([`/shop/product/checkout/${this.product.id}`],{
				queryParams: { "varient": this.product.variants[0].id },
